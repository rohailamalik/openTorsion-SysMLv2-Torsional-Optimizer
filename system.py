...
class System():
    ...
    def get_design_vars(self) -> dict:
        ...
        for i, comp in enumerate(self.components, start=1):
            ...
            if choices:
                if not isinstance(choices, list) or not all(isinstance(op, dict) for op in choices):
                    raise TypeError(f"{comp_name}: 'options' must be a list of dictionaries.")

                for j, opt in enumerate(choices, start=1):
                    ch_name = opt.get("name", f"Option {j}")
                    ch_params = opt.get("parameters")
                ...
                choices_names = [ch["name"] if isinstance(ch.get("name"), str) else f"Choice__{i}" for i, ch in enumerate(choices)]
